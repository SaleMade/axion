...
async function handlePresselsTotalPage(req, env){
  ...
  const totalSec=`<div style="background:#101d2e;border:1px solid #2b6cb0;border-radius:16px;padding:20px"><div style="font-size:16px;font-weight:800;margin-bottom:12px;color:#7aa2ff">TOTAL · todas as pressels</div>${cardsHtml(tot)}${vendTable(totVend)}</div>`;
  // TOTAL e "Chip pedindo troca" lado a lado, não empilhados. O aviso amarelo em cima empurrava as
  // pressels pra baixo e atrapalhava a leitura do começo da página. Em tela estreita o flex-wrap
  // desempilha sozinho (o de baixo vira o segundo).
  const topSec=_diagHtml
    ? `<div style="display:flex;gap:16px;align-items:flex-start;flex-wrap:wrap;margin-bottom:24px"><div style="flex:1 1 430px;min-width:0">${totalSec}</div><div style="flex:1 1 280px;min-width:0">${_diagHtml}</div></div>`
    : `<div style="margin-bottom:24px">${totalSec}</div>`;
